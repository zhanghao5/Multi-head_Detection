...
LIB_API void do_nms_sort_v3(detection *dets, int total, int classes, float thresh);
...
LIB_API int  diounms_sort_v1(detection *dets, int total, int classes, float thresh, NMS_KIND nms_kind, float beta1, float epsilon);
LIB_API void diounms_sort_v3(detection *dets, int total, int classes, float thresh, NMS_KIND nms_kind, float beta1);

// network.h
LIB_API float *network_predict(network net, float *input);
LIB_API float *network_head_predict(network net, int head, int version);
LIB_API float *network_predict_ptr(network *net, float *input);
LIB_API detection *get_network_boxes(network *net, int w, int h, float thresh, float hier, int *map, int relative, int *num, int letter);
LIB_API detection *get_head_boxes(network *net, int w, int h, float threah, float hier, int *map, int relative, int *num, int letter, int head, int version);
...
LIB_API detection *make_network_boxes_m(network *net, float thresh, int *num);
LIB_API detection *make_network_boxes_h(network *net, float thresh, int *num, int lb, int ub);
LIB_API void reset_rnn(network *net);
LIB_API float *network_predict_image(network *net, image im);
LIB_API float *network_predict_image_letterbox(network *net, image im);
LIB_API float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, list *heads, int n_heads, int gate, int max, int version, float epsilon);
