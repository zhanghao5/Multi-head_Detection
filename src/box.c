...
void do_nms_sort_v3(detection *dets, int total, int classes, float thresh)
{
    ...
    total = k + 1;
    // realloc size of 1 to size of 13
    for(i = 0; i < total; ++i){
        if(dets[i].classes == 1){
            float tmp = dets[i].prob[0];
            dets[i].prob = realloc(dets[i].prob, classes * sizeof(float));
            memset(dets[i].prob, 0, classes * sizeof(float));
            // classid mapping to 13 class array
            dets[i].prob[dets[i].classid] = tmp;
        }
    }
...
// only sort and compare two incremental learned classes
int diounms_sort_v1(detection *dets, int total, int classes, float thresh, NMS_KIND nms_kind, float beta1, float epsilon)
{
    if(total == 0){
        return -1;
    }
    ...
    // printf("diounms_sort() : total = %d\n", total);
    int cmp_cls[2] = {4,6};
    float probs[2] = {0.0, 0.0};
    int head = -1;

    for (k = 0; k < 2; ++k) {
        for (i = 0; i < total; ++i) {
            dets[i].sort_class = cmp_cls[k];
        }
        qsort(dets, total, sizeof(detection), nms_comparator_v3);
        for (i = 0; i < total; ++i)
        {
            if (dets[i].prob[cmp_cls[k]] == 0) continue;
            box a = dets[i].bbox;
            for (j = i + 1; j < total; ++j) {
                box b = dets[j].bbox;
                if (box_iou(a, b) > thresh && nms_kind == CORNERS_NMS)
                {
                    float sum_prob = pow(dets[i].prob[cmp_cls[k]], 2) + pow(dets[j].prob[k], 2);
                    float alpha_prob = pow(dets[i].prob[cmp_cls[k]], 2) / sum_prob;
                    float beta_prob = pow(dets[j].prob[cmp_cls[k]], 2) / sum_prob;
                    //dets[i].bbox.x = (dets[i].bbox.x*alpha_prob + dets[j].bbox.x*beta_prob);
                    //dets[i].bbox.y = (dets[i].bbox.y*alpha_prob + dets[j].bbox.y*beta_prob);
                    //dets[i].bbox.w = (dets[i].bbox.w*alpha_prob + dets[j].bbox.w*beta_prob);
                    //dets[i].bbox.h = (dets[i].bbox.h*alpha_prob + dets[j].bbox.h*beta_prob);
        ...
        probs[k] = dets[0].prob[cmp_cls[k]];

    }
    if(fabs(probs[0] - probs[1]) < epsilon){
        head = -1;
    }
...
void diounms_sort_v3(detection *dets, int total, int classes, float thresh, NMS_KIND nms_kind, float beta1)
{
    int i, j, k;
    k = total - 1;
    for (i = 0; i <= k; ++i) {
    ...
    total = k + 1;
    // realloc size of 1 to size of 13 only for one-class head
    for(i = 0; i < total; ++i){
        if(dets[i].classes == 1){
            float tmp = dets[i].prob[0];
            dets[i].prob = xrealloc(dets[i].prob, classes * sizeof(float));
            memset(dets[i].prob, 0, classes * sizeof(float));
            // classid mapping to 13 class array
            dets[i].prob[dets[i].classid] = tmp;
        }
