    ...
    while (get_current_iteration(net) < net.max_batches) {
        ...
        */
        ...
        if (calc_map && (iteration >= next_map_calc || iteration == net.max_batches)) {
            ...
            iter_map = iteration;
            mean_average_precision = validate_detector_map(datacfg, cfgfile, weightfile, 0.25, 0.5, 0, net.letter_box, &net_map, NULL, 0, 0, 0, 0, 0.0);// &net_combined);
...
float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net,
                            list *heads, int n_heads, int gate, int max, int version, float epsilon)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.txt");
    ...
    if (existing_net) {
        ...
        if(n_heads > 0 && heads->size > 0 && n_heads == heads->size){
            if (weightfile) {
                printf("load multiple weight file from %d incremental heads\n", n_heads);
                load_multi_weights(&net, weightfile, heads, n_heads, version);
            }
        }
        else{
            if(weightfile){
    ...
    // change net.n to base network size
    // store whole network size in netsize
    int netsize = 0;
    // head = -1 trigger all incremental heads
    int head = -1;
    if(gate || max){
        netsize = net.n;
        switch(version){
    ...
    // count where the fp coming from, base : incremental head?
    int *tp_for_thresh_per_class_b = (int *)xcalloc(classes, sizeof(int));
    int *tp_for_thresh_per_class_i = (int *)xcalloc(classes, sizeof(int));
    int *fp_for_thresh_per_class_b = (int *)xcalloc(classes, sizeof(int));
    int *fp_for_thresh_per_class_i = (int *)xcalloc(classes, sizeof(int));
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            if (args.type == LETTERBOX_DATA) {
                dets = get_network_boxes(&net, val[t].w, val[t].h, thresh, hier_thresh, 0, 1, &nboxes_b, letter_box);
            }
            else {
                dets = get_network_boxes(&net, 1, 1, thresh, hier_thresh, 0, 0, &nboxes_b, letter_box);
            }
            //detection *dets = get_network_boxes(&net, val[t].w, val[t].h, thresh, hier_thresh, 0, 1, &nboxes, letter_box); // for letter_box=1
            layer l = net.layers[net.n - 1];

            if (nms) {
                if (l.nms_kind == DEFAULT_NMS) do_nms_sort(dets, nboxes_b, l.classes, nms);
                else {
                	if(gate){
                		// doiunms_sort_v1 return the head seletctio result from class 4 and 6, bus and truck
                		// with epsilon comparison
                		head = diounms_sort_v1(dets, nboxes_b, l.classes, nms, l.nms_kind, l.beta_nms, epsilon);
                	}
                	// regular test, sort on all n classes
                	else{
                		diounms_sort(dets, nboxes_b, l.classes, nms, l.nms_kind, l.beta_nms);
                	}
                }
            ...
            int nboxes;
            if(gate){  // only seletction between two, need to update for select among more

                // printf("heads : %d\n", head);

                //forward_head based on heads selected
                int basesize = net.n;
                net.n = netsize;  // full size contails all incremental heads
                // printf("net.n = %d\n", net.n);

                network_head_predict(net, head, version);
                
                // get detection from head layers
                detection *dets_h;
                int nboxes_h = 0;
                dets_h = get_head_boxes(&net, 1, 1, thresh, hier_thresh, 0, 0, &nboxes_h, letter_box, head, version);
                       
                // for(int k = 0; k < nboxes_h; ++k){
                //     printf("head detection class 8 prob : %f,  class 10 prob : %f\n", dets_h[k].prob[8], dets[k].prob[10]);
                // }
            
                // merge dets & dets_h and do nms sort
                if(nboxes_b > 0){
                    nboxes = nboxes_b + nboxes_h;
                    dets = (detection*)xrealloc(dets, nboxes * sizeof(detection));
                    memcpy(dets + nboxes_b, dets_h, nboxes_h * sizeof(detection));
                ...
                // do_nms_sort_v3(dets, nboxes, classes, nms);
                layer head_l = net.layers[net.n - 1];                 
                diounms_sort_v3(dets, nboxes, classes, nms, head_l.nms_kind, head_l.beta_nms);
            ...
            else if(max)
            {
                ...
                //forward_head based on heads selected
                int basesize = net.n;
                net.n = netsize;  // full size
                // printf("net.n = %d\n", net.n);

                network_head_predict(net, -1, version);

                detection *dets_h;
                int nboxes_h = 0;
                dets_h = get_head_boxes(&net, 1, 1, thresh, hier_thresh, 0, 0, &nboxes_h, letter_box, -1, version);

                // for(int k = 0; k < nboxes_h; ++k){
                //     printf("head detection class 8 prob : %f,  class 10 prob : %f\n", dets_h[k].prob[8], dets[k].prob[10]);
                // }
            
                // merge dets & dets_h and do nms sort, handle if no detection
                if(nboxes_b > 0){
                    nboxes = nboxes_b + nboxes_h;
                    dets = (detection*)xrealloc(dets, nboxes * sizeof(detection));
                    memcpy(dets + nboxes_b, dets_h, nboxes_h * sizeof(detection));
            ...
            for (i = 0; i < nboxes; ++i) {
                ...
                for (class_id = 0; class_id < classes; ++class_id) {
                    ...
                    if (prob > 0) {
                        ...
                        detections[detections_count - 1].head = head;

                        int truth_index = -1;
                        float max_iou = 0;
                        // compare to all ground truth labels within a single image
                        // read form file and store in list *truth
                        for (j = 0; j < num_labels; ++j)
                        {
                            box t = { truth[j].x, truth[j].y, truth[j].w, truth[j].h };
                        ...
                        // not set difficult in cmd, shoul not working
                        else {
                        ...
                        if (prob > thresh_calc_avg_iou) {
                            int z, found = 0;
                            // since split one detection to up to n classes detections
                            // no same detection of other classeess found before this detection (iterate over all n classes)
                            ...
                            if (truth_index > -1 && found == 0) {
                                ...
                                if(detections[detections_count -1].head > -1)
                                    tp_for_thresh_per_class_i[class_id]++;
                                else tp_for_thresh_per_class_b[class_id]++;

                            }
                            // mainly for truth index = -1, classid not align, this classid's fp
                            else{
                                fp_for_thresh++;
                                fp_for_thresh_per_class[class_id]++;

                                if(detections[detections_count -1].head > -1)
                                    fp_for_thresh_per_class_i[class_id]++;
                                else fp_for_thresh_per_class_b[class_id]++;
    ...
    // SORT(detections)
    // descending order, compared with prob.
    ...
    for (i = 0; i < classes; ++i) {
        ...
        printf("Precision = %1.2f, Recall = %1.2f, F1 = %1.2f, avg IOU = %2.2f%% \n\n", class_precision, class_recall, 
            2*class_precision*class_recall/(class_precision + class_recall), avg_iou_per_class[i]);
    ...
    for (i = 0; i < classes; ++i){
        printf("class_id = %d,  FP_B= %d, FP_I = %d, FP_I %2.2f\n", i, fp_for_thresh_per_class_b[i], 
            fp_for_thresh_per_class_i[i], (float)fp_for_thresh_per_class_i[i]/(float)(fp_for_thresh_per_class_i[i] + fp_for_thresh_per_class_b[i]));
        printf("class_id = %d,  TP_B= %d, TP_I = %d, TP_I %2.2f\n", i, tp_for_thresh_per_class_b[i], 
            tp_for_thresh_per_class_i[i], (float)tp_for_thresh_per_class_i[i]/(float)(tp_for_thresh_per_class_i[i] + tp_for_thresh_per_class_b[i]));
    }
    ...
    free(fp_for_thresh_per_class_b);
    free(fp_for_thresh_per_class_i);
    free(tp_for_thresh_per_class_b);
    free(tp_for_thresh_per_class_i);


    fprintf(stderr, "Total Detection Time: %d Seconds\n", (int)(time(0) - start));
    gettimeofday(&tv2, NULL);
    long pr_time = (tv2.tv_sec - tv1.tv_sec) * 1000000 + (tv2.tv_usec - tv1.tv_usec);
    fprintf(stderr, "Total Detection Time: %ld usec\n", pr_time);
...
void run_detector(int argc, char **argv)
{
    ...
    // gate and max are mutual exlusive happen
    int gate = find_arg(argc, argv, "-gate");
    // max trigger all incremental head
    int max = find_arg(argc, argv, "-max");
    // n_heads, if 0, no incremental head, other # incremental head
    int n_heads = find_int_arg(argc, argv, "-n_heads", 0);
    // yolo version, default value is 4, epsilon default 10%
    int version = find_int_arg(argc, argv, "-version", 4);
    ...
    char *datacfg = argv[3];
    char *cfg = argv[4];
    // weights can be whole network weight or backbone weight
    ...
    if(argc < 6 + n_heads){
        fprintf(stderr, "usage: %s %s [map] [data] [cfg] [weights (backbone)] head_1 ... head_n -n_heads N -version # -gate|-max -epsilon 0.1] \n", argv[0], argv[1]);
        return;       
    }
    list *heads = make_list();
    for(int i = 0; i < n_heads; ++i){
        list_insert(heads, argv[6+i]);
    }
    ...
    else if (0 == strcmp(argv[2], "map")) validate_detector_map(datacfg, cfg, weights, thresh, iou_thresh, map_points, letter_box, NULL, heads, n_heads, gate, max, version, epsilon);
