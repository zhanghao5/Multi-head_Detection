...
void forward_network_head(network net, network_state state, int version)
{
    state.workspace = net.workspace;
    int i;
    int start, end;
...
float *network_head_predict(network net, int head, int version)
{
#ifdef GPU
    if(gpu_index >= 0)  return network_predict_head_gpu(net, head, version);
#endif
...
// for test bus and truck
    ...
    // printf("start forward layer: %d\n", state.index);

    state.truth = 0;
    state.train = 0;
    state.delta = 0;
    forward_network_head(net, state, version);

    // get_network_output upto the base head, if need output from the network
    // modify or add new function to get network output from all yolo layers
    float *out = get_network_output(net);
    return out;
}
...
int num_detections_h(network *net, float thresh, int lb, int ub)
{
    int i;
    int s = 0;
    for (i = lb ; i < ub; ++i) {
...
// copying from yolo_layer.c entry_index()
static int entry_mapping(layer l, int batch, int location, int entry)
{
...
// if yolo layer has different classes, use this function, for classes =1 and = 13 coexist
// l.classes can be 13 or 1 in this function
detection *make_network_boxes_m(network *net, float thresh, int *num)
{
    int i, j, n;
    int nboxes = num_detections(net, thresh);
    if (num) *num = nboxes;
    ...
    for (i = 0; i < net->n; ++i) {
        ...
        if (l.type == YOLO) {
            for (j = 0; j < l.w*l.h; ++j){
                for(n = 0; n < l.n; ++n){
                    int obj_index  = entry_mapping(l, 0, n*l.w*l.h + j, 4);
                    if(l.output[obj_index] > thresh){
                        dets[det_index].prob = (float*)calloc(l.classes, sizeof(float));
                        det_index++;
                    }
                }
            }
...
// make boxes from the branch/head yolo layer
detection *make_network_boxes_h(network *net, float thresh, int *num, int lb, int ub)
{
    int i;
    layer l = net->layers[net->n - 1];

    int nboxes = num_detections_h(net, thresh, lb, ub);
    if (num) *num = nboxes;
    detection* dets = (detection*)xcalloc(nboxes, sizeof(detection));
...
void fill_head_boxes(network *net, int w, int h, float thresh, float hier, int *map, int relative, detection *dets, int letter, int lb, int ub)
{
    int prev_classes = -1;
    int j;
    int total = 0;
    ...
    // printf("fill_head_boxes() : total detections = %d\n", total);
}
...
{   
    ...
    // detection *dets = make_network_boxes_m(net, thresh, num);
    // printf("base selection detection number : %d\n", *num);
    fill_network_boxes(net, w, h, thresh, hier, map, relative, dets, letter);
    return dets;
}

detection *get_head_boxes(network *net, int w, int h, float thresh, float hier, int *map, int relative, int *num, int letter, int head, int version){
    ...
    detection *dets = make_network_boxes_h(net, thresh, num, lb, ub);
    // printf("make network boxes() # of detection = %d\n", *num);
    fill_head_boxes(net, w, h, thresh, hier, map, relative, dets, letter, lb, ub);
    return dets;
