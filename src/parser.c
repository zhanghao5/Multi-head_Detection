...
layer parse_yolo(list *options, size_params params)
{
    ...
    int classid = option_find_int(options, "classid", -1);
    ...
    layer l = make_yolo_layer(params.batch, params.w, params.h, num, total, mask, classes, max_boxes, classid);
...
// inherit base head weights from n to 1 class
void save_convolutional_weights_i(layer l, FILE *fp)
{
    if(l.binary){
        //save_convolutional_weights_binary(l, fp);
        //return;
...
#endif
    int num = l.nweights;
    int onesize = l.nweights / (l.n * l.size * l.size);
    ...
    if (l.batch_normalize){
        ...
        for(int j = 0; j < onesize; j++){
            for(int i = 0; i < 3; i++){
                fwrite(l.weights + (6  + i*18), sizeof(float), 1, fp);
                fwrite(l.weights + (13 + i*18), sizeof(float), 5, fp);
            }
        }
...
void save_weights_upto(network net, char *filename, int cutoff)
{
    ...
    for(i = 0; i < net.n && i < cutoff; ++i){
        ...
        if (l.type == CONVOLUTIONAL && l.share_layer == NULL) {
            ...
            // printf("layer : %d   ", i);
            // save_convolutional_weights_i(l, fp);
...
void load_convolutional_weights_backbone(layer l, FILE *fp, FILE ** fp_heads, int n_heads)
{
    if(l.binary){
        //load_convolutional_weights_binary(l, fp);
        //return;
    ...
    int read_bytes;
    read_bytes = fread(l.biases, sizeof(float), l.n, fp);
    // sync file pointers of heads
    for(int i = 0; i < n_heads; ++i){
            fseek(fp_heads[i], l.n * sizeof(float), SEEK_CUR);
    }
    ...
    //sync
    for(int i = 0; i < n_heads; ++i){
        fseek(fp_heads[i], num * sizeof(float), SEEK_CUR);
    }
...
void load_shortcut_weights(layer l, FILE *fp, FILE ** fp_heads, int n_heads)
{
    int num = l.nweights;
    int read_bytes;
    read_bytes = fread(l.weights, sizeof(float), num, fp);
    if(n_heads > 0 && fp_heads != NULL){
        printf("load_shortcut_weights(): sync ...\n ");
        for(int i = 0; i < n_heads; ++i){
            fseek(fp_heads[i], num * sizeof(float), SEEK_CUR);
        }
    }
...
void load_weights_upto(network *net, char *filename, int cutoff)
{
    ...
    *net->cur_iteration = get_current_batch(*net);
    ...
    for(i = 0; i < net->n && i < cutoff; ++i){
        ...
        if (l.type == SHORTCUT && l.nweights > 0) {
            load_shortcut_weights(l, fp, NULL, 0);
        }
        if(l.type == CONNECTED){
...
void load_multi_weights(network *net, char *filename, list *heads, int n_heads, int version)
{
#ifdef GPU
    if(net->gpu_index >= 0){
        cuda_set_device(net->gpu_index);
    ...
    switch(version){
        ...
        case 4:
            ...
            break;
        default:
            printf("load_multi_weights(): wrong version number ! \n");
            b_bound = net->n;
    ...
    FILE *fp = fopen(filename, "rb");
    if(!fp) file_error(filename);
    // base head in position 0
    FILE **fp_heads = (FILE*)malloc(n_heads);
    for(int i = 0; i < n_heads; ++i){
        fp_heads[i] = fopen(heads->front->val, "rb");
        if(!fp_heads[i]) file_error(heads->front->val);
        list_pop_front(heads);
    }
    ...
    // sync with fp
    for(int i = 0; i < n_heads; ++i){
        fseek(fp_heads[i], 3 * sizeof(int), SEEK_CUR);
    }
    ...
    if ((major * 10 + minor) >= 2) {
        ...
        uint64_t iseen = 0;
        fread(&iseen, sizeof(uint64_t), 1, fp);
        //sync file pointer with fp
        for(int i = 0; i < n_heads; ++i){
            fseek(fp_heads[i], sizeof(uint64_t), SEEK_CUR);
        }
        *net->seen = iseen;
    }
    else {
        ...
        //sync file pointer with fp
        for(int i = 0; i < n_heads; ++i){
            fseek(fp_heads[i], sizeof(uint32_t), SEEK_CUR);
        }
        *net->seen = iseen;
    }
    int transpose = (major > 1000) || (minor > 1000);

    // setup bound loading for each incremental head
    int *bounds = (int*)malloc(n_heads + 1);
    for(int j = 0; j < n_heads + 1; ++j){
        bounds[j] = b_bound + j * 33;
    }

    int i;
    printf("load_multi_weights(): net->n = %d\n", net->n);
    for(i = 0; i < net->n ; ++i){
        layer l = net->layers[i];
        if (l.dontload) continue;
        if(l.type == CONVOLUTIONAL && l.share_layer == NULL){
            if(i < b_bound){
                //load backbone
                if(i < b_cut){
                    load_convolutional_weights_backbone(l, fp, fp_heads, n_heads);
                    // printf("layer type 1\n");
                }
                else {
                    load_convolutional_weights(l, fp);
                ...
                for(int j = 0; j < n_heads; ++j){
                    if(i >= bounds[j] && i < bounds[j+1]){
                        // printf("layer : %d,  bound: %d\n", i, j);
                        load_convolutional_weights(l, fp_heads[j]);
                        // printf("layer type 3 : head : %d\n", j);
                    }
                }
            }
            
        }
        if(version == 4 && l.type == SHORTCUT && l.nweights > 0){
            load_shortcut_weights(l, fp, fp_heads, n_heads);
        }
        if(l.type == CONNECTED){
            load_connected_weights(l, fp, transpose);
        }
