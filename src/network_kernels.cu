...
void forward_network_head_gpu(network net, network_state state, int start, int end)
{
    //cudaDeviceSynchronize();
    //printf("\n");
    state.workspace = net.workspace;
...
float *network_predict_head_gpu(network net, int head, int version)
{
    if (net.gpu_index != cuda_get_device())
        cuda_set_device(net.gpu_index);
    // int size = get_network_input_size(net) * net.batch;
    ...
    // state.index = 0;
    state.net = net;
    // head input is from base forward 74 61 36 conv layers
    //state.input = cuda_make_array(input, size);   // memory will be allocated in the parse_network_cfg_custom()
    // state.input = net.input_state_gpu;
    // memcpy(net.input_pinned_cpu, input, size * sizeof(float));
    // cuda_push_array(state.input, net.input_pinned_cpu, size);
    ...
    state.train = 0;
    state.delta = 0;
    forward_network_head_gpu(net, state, start, end);
